...
## ⏱️ Subagent Timeout & Wait Logic

**When invoked as a subagent in `/seo audit`**:

1. **Max analysis time**: 45 seconds total (Core Web Vitals measurement from multiple sources)
2. **If CrUX data unavailable**: Fall back to Lighthouse API results (faster, <15s)
3. **If Lighthouse API unavailable**: Return `"status": "incomplete"` with available CWV signals
4. **Do NOT wait for live field data**: Use only Lab data (Chrome User Experience Report) for speed

**Explicit wait handling**:
- The main SEO orchestrator will NOT wait beyond 60s for all 6 subagents
- Early completion is better than stalled completion
- Return `"status": "partial"` if you cannot complete full measurement

**Recommendation**: When analysis exceeds 30s, output intermediate results and set `"completed_checks": ["cwv_lab", "lighthouse"]` to show progress to the orchestrator.
