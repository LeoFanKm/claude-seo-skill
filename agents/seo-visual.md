...
**Critical**: Set explicit timeouts to prevent blocking the main orchestration:

```bash
# Use domcontentloaded instead of networkidle for faster completion
page.goto(url, wait_until='domcontentloaded', timeout=30000)  # 30s max

# Add per-operation timeout
page.wait_for_load_state('networkidle', timeout=10000)  # Optional, 10s fallback
```

**If Playwright is unavailable**: Return `{"error": "playwright_unavailable", "fallback": "webfetch"}`. The orchestrator will fall back to static HTML analysis via WebFetch.

**If analysis exceeds 30s**: Return partial results with `"timeout": true`. The orchestrator will accept incomplete visual data rather than stall the entire audit.

**Recommendation for subagent coordination**: When invoked in a parallel audit (`/seo audit`), use `/compact` output mode to reduce token overhead and allow sibling agents to complete faster.
